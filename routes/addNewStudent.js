...
var addNewStudent = function (req, res) {
    ...
    /*var paths = req.files.map(file => {
        profileUrl = "http://10.0.2.2:6000/public/" +file.filename;
    });*/
    var profileUrl="http://10.0.2.2:6000/public/" + req.files['profile'][0].filename;

    if (class_id != null && student_name != null && mobile_no != null && gender != null && dob != null && standard_id != null && batch != null && email != null && joining_date != null && updated_date != null
                                                ...
                                                connection.rollback(function () {
                                                   ...
                                                   connection.query('INSERT INTO parents(FatherName, MotherName, Mobile, Email, Address, JoinDate, UpdateDate) VALUES(?,?,?,?,?,?,?)',
                                                       [father_name, mother_name, parent_mobileNo, parent_email, address, joining_date, updated_date],function (err, result1) {
                                                           ...
                                                           connection.query('INSERT INTO parent_students(StudentId, ParentId) VALUES(?,?)',[studentId, parentId], function (err, result) {
                                                                   if (err) {
                                                                       connection.rollback(function (){
                                                                           connection.release();
                                                                           res.send(JSON.stringify({ error: true, message: err.message }));
                                                                   ...
                                                                   connection.query('INSERT INTO class_students(ClassId, StudentId) VALUES(?,?)',[class_id, studentId], function (err, result) {
                                                                           if (err) {
                                                                               connection.rollback(function () {
                                                                                   connection.release();
                                                                                   res.send(JSON.stringify({ error: true, message: err.message }));
                                                                           ...
                                                                           connection.query('INSERT INTO class_parents(ParentId, ClassId) VALUES(?,?)',[parentId, class_id], function (err, result) {
                                                                                   if (err) {
                                                                                       connection.rollback(function () {
                                                                                           connection.release();
                                                                                           res.send(JSON.stringify({ error: true, message: err.message }));
